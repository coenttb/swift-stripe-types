    ...
    public var body: some URLRouting.Router<Stripe.PaymentIntents.API> {
      OneOf {
        ...
        URLRouting.Route(.case(Stripe.PaymentIntents.API.capture)) {
          ...
          Path { "capture" }
          Body(
            .form(Stripe.PaymentIntents.Capture.Request.self, decoder: .stripe, encoder: .stripe)
          )
        }
        ...
        URLRouting.Route(.case(Stripe.PaymentIntents.API.confirm)) {
          ...
          Path { "confirm" }
          Body(
            .form(Stripe.PaymentIntents.Confirm.Request.self, decoder: .stripe, encoder: .stripe)
          )
        }
