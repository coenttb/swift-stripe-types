        ...
        public var body: some URLRouting.Router<Stripe.Payouts.API> {
            OneOf {
                Route(.case(Stripe.Payouts.API.create)) {
                    ...
                    Path.v1
                    Path.payouts
                    Body(.form(Stripe.Payouts.Create.Request.self, decoder: .stripe, encoder: .stripe))
                }
                ...
                Route(.case(Stripe.Payouts.API.update)) {
                    ...
                    Path.payouts
                    Path { Parse(.string.representing(Stripe.Payouts.Payout.ID.self)) }
                    Body(.form(Stripe.Payouts.Update.Request.self, decoder: .stripe, encoder: .stripe))
                }
                ...
                Route(.case(Stripe.Payouts.API.list)) {
                    ...
                    Parse(.memberwise(Stripe.Payouts.List.Request.init)) {
                        Query {
                            Optionally {
                                Field("arrival_date") { Parse(.string.representing(Stripe.DateFilter.self)) }
                            }
                            Optionally {
                            ...
                            Optionally {
                                Field("status") { Parse(.string.representing(Stripe.Payouts.Payout.Status.self)) }
                            }
                        }
                    }
                }
                ...
                Route(.case(Stripe.Payouts.API.reverse)) {
                    ...
                    Path { Parse(.string.representing(Stripe.Payouts.Payout.ID.self)) }
                    Path.reverse
                    Body(.form(Stripe.Payouts.Reverse.Request.self, decoder: .stripe, encoder: .stripe))
                }
            }
        }
    }
