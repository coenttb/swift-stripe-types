...
extension Stripe.Billing.Customer.Balance {
    ...
    public enum API: Equatable, Sendable {
        ...
        case update(customerId: Stripe.Customers.Customer.ID, id: Transaction.ID, request: Update.Request)
        ...
        public var body: some URLRouting.Router<Stripe.Billing.Customer.Balance.API> {
            OneOf {
                URLRouting.Route(.case(Stripe.Billing.Customer.Balance.API.create)) {
                    ...
                    Body(.form(Stripe.Billing.Customer.Balance.Create.Request.self, decoder: .stripe, encoder: .stripe))
                }
                ...
                URLRouting.Route(.case(Stripe.Billing.Customer.Balance.API.update)) {
                    ...
                    Path.balance_transactions
                    Path { Parse(.string.representing(Stripe.Billing.Customer.Balance.Transaction.ID.self)) }
                    Body(.form(Stripe.Billing.Customer.Balance.Update.Request.self, decoder: .stripe, encoder: .stripe))
                }
...
extension Path<PathBuilder.Component<String>> {
    ...
    nonisolated(unsafe) package static let balance_transactions = Path { "balance_transactions" }
}
