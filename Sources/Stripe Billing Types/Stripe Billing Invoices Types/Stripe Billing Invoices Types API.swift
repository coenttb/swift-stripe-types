    ...
    public var body: some URLRouting.Router<Stripe.Billing.Invoices.API> {
      OneOf {
        ...
        URLRouting.Route(.case(Stripe.Billing.Invoices.API.create)) {
          ...
          Path.invoices
          Body(
            .form(Stripe.Billing.Invoices.Create.Request.self, decoder: .stripe, encoder: .stripe)
          )
        }
        ...
        URLRouting.Route(.case(Stripe.Billing.Invoices.API.update)) {
          ...
          Path { Parse(.string.representing(Stripe.Billing.Invoice.ID.self)) }
          Body(
            .form(Stripe.Billing.Invoices.Update.Request.self, decoder: .stripe, encoder: .stripe)
          )
        }
            ...
            URLRouting.Query {
              Optionally {
                Field("collection_method") {
                  Parse(.string.representing(Stripe.Billing.Invoice.CollectionMethod.self))
                }
              }
              Optionally {
        ...
        URLRouting.Route(.case(Stripe.Billing.Invoices.API.finalize)) {
          ...
          Path.finalize
          Body(
            .form(Stripe.Billing.Invoices.Finalize.Request.self, decoder: .stripe, encoder: .stripe)
          )
        }
