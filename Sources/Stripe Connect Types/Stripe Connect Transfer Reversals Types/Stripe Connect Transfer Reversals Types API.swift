...
extension Stripe.Connect.Transfer.Reversals {
    ...
    public enum API: Equatable, Sendable {
        ...
        case retrieve(transferId: Stripe.Connect.Transfer.ID, reversalId: Stripe.Connect.Transfer.Reversal.ID)
        // https://docs.stripe.com/api/transfer_reversals/update.md
        case update(transferId: Stripe.Connect.Transfer.ID, reversalId: Stripe.Connect.Transfer.Reversal.ID, request: Update.Request)
        ...
        public var body: some URLRouting.Router<Stripe.Connect.Transfer.Reversals.API> {
            OneOf {
                Route(.case(Stripe.Connect.Transfer.Reversals.API.create)) {
                    ...
                    Path { "reversals" }
                    Body(.form(Stripe.Connect.Transfer.Reversals.Create.Request.self, decoder: .stripe, encoder: .stripe))
                }
                ...
                Route(.case(Stripe.Connect.Transfer.Reversals.API.update)) {
                    ...
                    Path { "reversals" }
                    Path { Parse(.string.representing(Stripe.Connect.Transfer.Reversal.ID.self)) }
                    Body(.form(Stripe.Connect.Transfer.Reversals.Update.Request.self, decoder: .stripe, encoder: .stripe))
                }
