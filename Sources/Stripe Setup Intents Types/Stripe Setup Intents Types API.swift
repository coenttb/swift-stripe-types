...
extension Stripe.Setup.Intents {
    ...
    public enum API: Equatable, Sendable {
        ...
        case verifyMicrodeposits(id: Stripe.Setup.Intent.ID, request: Stripe.Setup.Intents.VerifyMicrodeposits.Request)
    }
}
        ...
        public var body: some URLRouting.Router<Stripe.Setup.Intents.API> {
            OneOf {
                ...
                URLRouting.Route(.case(Stripe.Setup.Intents.API.list)) {
                    ...
                    Parse(.memberwise(Stripe.Setup.Intents.List.Request.init)) {
                        URLRouting.Query {
                            ...
                            Optionally {
                                Field("payment_method") { Parse(.string.representing(Stripe.PaymentMethods.PaymentMethod.ID.self)) }
                            }
                            Optionally {
                ...
                URLRouting.Route(.case(Stripe.Setup.Intents.API.verifyMicrodeposits)) {
                    ...
                    Body(.form(Stripe.Setup.Intents.VerifyMicrodeposits.Request.self, decoder: .stripe, encoder: .stripe))
                }
            }
        }
    }
