        ...
        public var body: some URLRouting.Router<Stripe.Products.ShippingRates.API> {
            OneOf {
                Route(.case(Stripe.Products.ShippingRates.API.create)) {
                    ...
                    Path.v1
                    Path.shippingRates
                    Body(.form(Stripe.Products.ShippingRates.Create.Request.self, decoder: .stripe, encoder: .stripe))
                }
                ...
                Route(.case(Stripe.Products.ShippingRates.API.update)) {
                    ...
                    Path.shippingRates
                    Path { Parse(.string.representing(Stripe.Products.Shipping.Rate.ID.self)) }
                    Body(.form(Stripe.Products.ShippingRates.Update.Request.self, decoder: .stripe, encoder: .stripe))
                }
...
extension Path<PathBuilder.Component<String>> {
    nonisolated(unsafe) public static let shippingRates = Path {
        "shipping_rates"
    }
}
