    ...
    public var body: some URLRouting.Router<Stripe.Connect.Accounts.API> {
      OneOf {
        Route(.case(Stripe.Connect.Accounts.API.create)) {
          ...
          Path.accounts
          Body(
            .form(Stripe.Connect.Accounts.Create.Request.self, decoder: .stripe, encoder: .stripe)
          )
        }
        ...
        Route(.case(Stripe.Connect.Accounts.API.update)) {
          ...
          Path { Parse(.string.representing(Stripe.Connect.Account.ID.self)) }
          Body(
            .form(Stripe.Connect.Accounts.Update.Request.self, decoder: .stripe, encoder: .stripe)
          )
        }
        ...
        Route(.case(Stripe.Connect.Accounts.API.reject)) {
          ...
          Path.reject
          Body(
            .form(Stripe.Connect.Accounts.Reject.Request.self, decoder: .stripe, encoder: .stripe)
          )
        }
      }
    }
