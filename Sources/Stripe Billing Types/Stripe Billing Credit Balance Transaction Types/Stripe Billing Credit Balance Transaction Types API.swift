        ...
        public var body: some URLRouting.Router<Stripe.Billing.Credit.Balance.API> {
            OneOf {
                URLRouting.Route(.case(Stripe.Billing.Credit.Balance.API.retrieve)) {
                    ...
                    Path { Parse(.string.representing(Stripe.Billing.Credit.Balance.Transaction.ID.self)) }
                }
...
extension Path<PathBuilder.Component<String>> {
    nonisolated(unsafe) package static let credit_balance_transactions = Path { "credit_balance_transactions" }
}
