...
extension Stripe.Billing.UsageRecords {
    ...
    public enum API: Equatable, Sendable {
        ...
        case create(subscriptionItemId: Stripe.Billing.SubscriptionItems.SubscriptionItem.ID, request: Create.Request)
        // https://docs.stripe.com/api/usage_records/list
        case list(subscriptionItemId: Stripe.Billing.SubscriptionItems.SubscriptionItem.ID, request: List.Request)
    }
}
        ...
        public var body: some URLRouting.Router<Stripe.Billing.UsageRecords.API> {
            OneOf {
                URLRouting.Route(.case(Stripe.Billing.UsageRecords.API.create)) {
                    ...
                    Path { Parse(.string.representing(Stripe.Billing.SubscriptionItems.SubscriptionItem.ID.self)) }
                    Path.usage_records
                    Body(.form(Stripe.Billing.UsageRecords.Create.Request.self, decoder: .stripe, encoder: .stripe))
                }
