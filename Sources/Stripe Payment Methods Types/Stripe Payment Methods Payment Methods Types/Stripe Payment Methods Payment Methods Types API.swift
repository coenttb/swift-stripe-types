...
extension Stripe.PaymentMethods.PaymentMethods {
    ...
    public enum API: Equatable, Sendable {
        ...
        case retrieveCustomer(customerId: Stripe.Customers.Customer.ID, paymentMethodId: Stripe.PaymentMethods.PaymentMethod.ID)
        ...
        public var body: some URLRouting.Router<Stripe.PaymentMethods.PaymentMethods.API> {
            OneOf {
                Route(.case(Stripe.PaymentMethods.PaymentMethods.API.create)) {
                    ...
                    Body(.form(Stripe.PaymentMethods.PaymentMethods.Create.Request.self, decoder: .stripe, encoder: .stripe))
                }
                ...
                Route(.case(Stripe.PaymentMethods.PaymentMethods.API.retrieve)) {
                    ...
                    Path.v1
                    Path.payment_methods
                    Path { Parse(.string.representing(Stripe.PaymentMethods.PaymentMethod.ID.self)) }
                }
                ...
                Route(.case(Stripe.PaymentMethods.PaymentMethods.API.update)) {
                    ...
                    Path.payment_methods
                    Path { Parse(.string.representing(Stripe.PaymentMethods.PaymentMethod.ID.self)) }
                    Body(.form(Stripe.PaymentMethods.PaymentMethods.Update.Request.self, decoder: .stripe, encoder: .stripe))
                }
                ...
                Route(.case(Stripe.PaymentMethods.PaymentMethods.API.listCustomer)) {
                    ...
                    Path { Parse(.string.representing(Stripe.Customers.Customer.ID.self)) }
                    Path.payment_methods
                    Parse(.memberwise(Stripe.PaymentMethods.PaymentMethods.List.Customer.Request.init)) {
                        Query {
                            Optionally {
                                Field("type") { Parse(.string) }
                            }
                ...
                Route(.case(Stripe.PaymentMethods.PaymentMethods.API.attach)) {
                    ...
                    Path { Parse(.string.representing(Stripe.PaymentMethods.PaymentMethod.ID.self)) }
                    Path.attach
                    Body(.form(Stripe.PaymentMethods.PaymentMethods.Attach.Request.self, decoder: .stripe, encoder: .stripe))
                }
