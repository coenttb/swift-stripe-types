        ...
        public var body: some URLRouting.Router<Stripe.Billing.Plans.API> {
            OneOf {
                ...
                URLRouting.Route(.case(Stripe.Billing.Plans.API.create)) {
                    ...
                    Path.v1
                    Path.plans
                    Body(.form(Stripe.Billing.Plans.Create.Request.self, decoder: .stripe, encoder: .stripe))
                }
                ...
                URLRouting.Route(.case(Stripe.Billing.Plans.API.update)) {
                    ...
                    Path.plans
                    Path { Parse(.string.representing(Stripe.Billing.Plan.ID.self)) }
                    Body(.form(Stripe.Billing.Plans.Update.Request.self, decoder: .stripe, encoder: .stripe))
                }
...
extension Path<PathBuilder.Component<String>> {
    nonisolated(unsafe) public static let plans = Path {
        "plans"
    }
}
