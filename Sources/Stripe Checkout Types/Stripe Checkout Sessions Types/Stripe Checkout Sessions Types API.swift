...
extension Stripe.Checkout.Sessions {
    ...
    public enum API: Equatable, Sendable {
        ...
        case update(id: Stripe.Checkout.Session.ID, request: Stripe.Checkout.Sessions.Update.Request)
        ...
        case lineItems(id: Stripe.Checkout.Session.ID, request: Stripe.Checkout.Sessions.LineItems.Request)
    }
}
        ...
        public var body: some URLRouting.Router<Stripe.Checkout.Sessions.API> {
            OneOf {
                URLRouting.Route(.case(Stripe.Checkout.Sessions.API.create)) {
                    ...
                    Path.checkout
                    Path.sessions
                    Body(.form(Stripe.Checkout.Sessions.Create.Request.self, decoder: .stripe, encoder: .stripe))
                }
                ...
                URLRouting.Route(.case(Stripe.Checkout.Sessions.API.update)) {
                    ...
                    Path.sessions
                    Path { Parse(.string.representing(Stripe.Checkout.Session.ID.self)) }
                    Body(.form(Stripe.Checkout.Sessions.Update.Request.self, decoder: .stripe, encoder: .stripe))
                }
...
extension Path<PathBuilder.Component<String>> {
    ...
    nonisolated(unsafe) public static let checkout = Path {
        "checkout"
    }
}
